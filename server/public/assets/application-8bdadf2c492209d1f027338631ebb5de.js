...
(function() {
  $(document).on('ready page:load', function() {
    $('#btnAddClientContact').popover({
      ...
      placement: 'left',
      content: function() {
        return $('#popoverAddClientContact').html();
      }
    });
    $('#clientContacts').on('click', '#btnSaveAddClientContact', function(e) {
      e.preventDefault();
      $('#clientContactsEmptyRecord').remove();
      $('#clientContactsSampleRecord').show(500);
      return $('#btnAddClientContact').popover('hide');
    });
    $(window).scroll(function(e) {
      return $('#form-nav .nav').css({
        top: 60
      });
    });
    return $('#saveAndSubmitModal').on('hidden', function() {
