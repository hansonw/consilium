...
App.factory 'MimeType', [->
  path =
    extname: (filename) ->
      filename.substr(filename.lastIndexOf(".")) if filename.lastIndexOf(".") > 0
  ...
  MimeType =
    ...
    lookup: (fname, include_charset, default_mime_type) ->
      ext = undefined
      charset = @charset
      include_charset = false  if include_charset is `undefined`
      if typeof include_charset is "string"
        charset = include_charset
        include_charset = true
      if path.extname isnt `undefined`
      ...
      ext = fname  if ext is ""
      if @catalog[ext] isnt `undefined`
        if include_charset is true and @catalog[ext].indexOf("text/") is 0 and @catalog[ext].indexOf("charset") < 0
          return @catalog[ext] + "; charset=" + charset
        else
          return @catalog[ext]
      else if default_mime_type isnt `undefined`
        return default_mime_type + "; charset=" + charset  if include_charset is true and default_mime_type.indexOf("text/") is 0
    ...
    set: (exts, mime_type_string) ->
      ...
      if exts.indexOf(",")
        exts.split(",").forEach (ext) ->
          ...
          result = false  if self.catalog[ext] isnt mime_type_string

      else
        result = (self.catalog[exts] is mime_type_string)
      result

    del: (ext) ->
      delete @catalog[ext]

      @catalog[ext] is `undefined`
    ...
    forEach: (callback) ->
      ...
      for ext of self.catalog
        callback ext, self.catalog[ext]  if self.catalog.hasOwnProperty(ext)
      self.catalog
  ...
  return {
    ...
    catalog: MimeType.catalog
    lookup: MimeType.lookup
    set: MimeType.set
    del: MimeType.del
    forEach: MimeType.forEach
  }
]
